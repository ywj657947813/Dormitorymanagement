...
function insertOrder(){
    ...
    $.ajax({
	       ...
	       success:function(result){
	           ...
	           if(result.msg=="true"){
	               alert("提交成功！");
	               window.location.href="Servicepextract.jsp";
	           }
    	...
    	<table class="table table-bordered">
			...
			<tr>
				<td align="center" colspan="4">
				    ...
				    <input type="text" name="people" value="service" hidden/>
             ...
             <c:when test="${cart.cartItems.size() >0}" >   
				...
				<c:forEach var="message" items="${cart.cartItems}">
					<tr>
						...
						<td align="center"><a href="cartInsert?people=service&action=removeCart&deleteid=${message.dormid}${message.product.pid}">删除</a></td>
					</tr>
				</c:forEach>
				<tr>
					<td align="center" colspan="4">
						...
						<c:if test="${cart.cartItems.size() >0}">
							<a href="cartInsert?people=service&action=clearCart"class="btn btn-default btn-lg active" onClick="return confirm('确定清空吗?');">清空清单</a>
						 </c:if>
						 <c:if test="${cart.cartItems.size() ==0}">
